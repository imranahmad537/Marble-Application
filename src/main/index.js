...
function saveOrderInvoice(payload, type) {
  ...
  if (type === 'full') {
    const insertInvoice = db.prepare(`
      INSERT INTO full_customer_invoice (created_at)
      VALUES (?)
    `);
    const result = insertInvoice.run(new Date().toISOString());
    invoiceId = result.lastInsertRowid;

  } else if (type === 'partial') {
    const insertInvoice = db.prepare(`
      INSERT INTO partial_customer_invoice (cnic, contact, created_at)
      VALUES (?, ?, ?)
    `);
...
ipcMain.handle('get-customer-order-invoice', () => {
  const fullOrder = db.prepare(`
    SELECT * FROM  order_invoice_product
    `).all();
    return fullOrder;
})
...
//--------------------------------------------------------------------------------
  ...
  ipcMain.handle("add-product", (event, product) => {
    try {
      db.prepare(`INSERT INTO products (name, rate, type) VALUES (?, ?, ?)`)
        .run(product.name, parseFloat(product.rate), product.type || null);
      return { success: true };
    } catch (error) {
  ...
  ipcMain.handle("update-product", (event, product) => {
    try {
      db.prepare(`UPDATE products SET rate = ?, type = ? WHERE name = ?`)
        .run(parseFloat(product.rate), product.type || null, product.name);
      return { success: true };
    } catch (error) {
